...
void Visualization::publishLink(){
    ...
    if (linkAvailable){
        ...
        for (uint i = 0; i < n_link; i++) {
            ...
            tf_broadcaster.sendTransform(tf::StampedTransform(trans, ros::Time::now(), "map", name.c_str()));
...
void Visualization::publishForceList() {
    ...
    if (cable_force.size() != 0 && cable_force.size() == cable_start.size()){
        ...
        line_list_action.header.frame_id = "map";
        ...
        line_list_reaction.header.frame_id = "map";
