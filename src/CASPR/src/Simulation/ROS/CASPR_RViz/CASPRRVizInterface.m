...
classdef CASPRRVizInterface < CableActuatorInterfaceBase
    properties (Access = private)        
        ...
        % Velocity
        link_vel_pub;               % Pub for velocity of link
        link_vel_msg;              % Message type for the link
    end
    ...
    properties (Access = private, Constant = true)  
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
          ...
          % Velocity of link
        link_vel_topic = '/joint_vel';
        link_vel_msg_type = 'std_msgs/Float32MultiArray';
    ...
    methods (Access = public)
        ...
        function linkInfoSend(obj, cdpr)
            ...
            % Start velocity
            for t = 1:length(cdpr.time)
                link_vel = [];
%                 cdpr.bodyModel.update(cdpr.traj.q{t}, cdpr.traj.q_dot{t}, cdpr.traj.q_ddot{t},zeros(size(cdpr.traj.q_dot{t})));
                link_vel(1:3) = cdpr.bodyModel.bodies{7}.R_0k*cdpr.bodyModel.bodies{7}.v_OG;
                link_vel(4:6) = cdpr.bodyModel.bodies{1}.R_0k*cdpr.bodyModel.bodies{1}.w;
                obj.link_vel_msg.Data = link_vel;
                send(obj.link_vel_pub, obj.link_vel_msg);
            end
            % end velocity
            
%              cdpr.bodyModel.update(cdpr.bodyModel.bodies{5}.joint.q,cdpr.bodyModel.bodies{5}.joint.q_dot, cdpr.bodyModel.bodies{5}.joint.q_ddot,zeros(size(cdpr.bodyModel.bodies{5}.joint.q_dot)));
%              link_vel(1:3,1) = cdpr.bodyModel.bodies{5}.R_0k*cdpr.bodyModel.bodies{5}.v_OG;
            ...
            % Velocity
            len = length(cdpr.bodyModel.bodies{i}.joint.q_dot);
%             link_vel = zeros(len,cdpr.numLinks); 
            ...
            for i = 1:cdpr.numLinks
                ...
                length(cdpr.bodyModel.bodies{i}.joint.q_dot);
                % Velocity
%                 link_vel = [link_vel, cdpr.bodyModel.bodies{i}.joint.q_dot];
            end
    ...
    methods (Access = private)        
        ...
        function createPublisher(obj)
            try
                ...
                [obj.link_vel_pub,obj.link_vel_msg] = rospublisher(obj.link_vel_topic, obj.link_vel_msg_type);
            catch
